...
#include <cstdint>
#include <iosfwd>
// Forward declare PaletteColor for SPRED
...
public:
    ...
    /// @param outIDs Output array (must hold at least maxIDs entries)
    /// @param maxIDs Maximum number of IDs to write
    /// @return Number of palettes in category
    static int32_t getPalettesByCategory(const char* category, uint8_t* outIDs, int32_t maxIDs);
...
private:
    ...
    static bool parseBinary(std::ifstream& file);
